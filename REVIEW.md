...
| Group fan-out uses chanx's *unvalidated* passthrough | `_send_now`, `VOTEIT_WS_FAST_FANOUT` | The typed path re-validates per recipient — ~3× CPU in a 500-participant meeting, for messages already type-safe at the publisher. Flag exists to switch. |
