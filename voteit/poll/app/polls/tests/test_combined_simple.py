...
class SimpleTests(TestCase):
    def setUp(self):
        ...
        self.poll = Poll.objects.create(electoral_register=self.er, method_name="combined_simple")
    ...
    def test_result(self):
        ...
        self.poll.ongoing()
        self.poll.votes.create(user=ua, vote=f'{{"yes": [{prop.pk}], "no": [{prop2.pk}]}}')
        ...
        self.assertEqual(
            self.poll.proposals.get(pk=prop.pk).state,
            ProposalWf.APPROVED
        )
        self.assertEqual(
            self.poll.proposals.get(pk=prop2.pk).state,
            ProposalWf.DENIED
        )
