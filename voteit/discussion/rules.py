...
from voteit.meeting.rules import is_moderator, is_not_archived, is_discusser, is_participant
...
def can_view_discussion_post(user: User, discussion_post: DiscussionPost):
    ...
    try:
        ...
        return
    return is_moderator(user, meeting) or is_non_private_ai(user, discussion_post.agenda_item) and is_participant(user, meeting)
