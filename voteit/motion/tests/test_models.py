...
class MotionProcessTests(TestCase):
    ...
    def _export_fixture(self):
        ...
        self.m1 = self.mp.motions.create(
            author=self.jane,
            title="Equal rights for all",
            body="It's time for a change",
        )
        self.m1.publish()
        self.m1.accept()
        self.m1.save()
        self.m1_p1 = self.m1.proposals.create(body="All men are created equal")
        self.m2 = self.mp.motions.create(
            author=self.tarzan, title="Ba na nas!", body="Bananas for monkeys!!1"
        )
        self.m2.publish()
        self.m2.accept()
        self.m2.save()
        self.m2_p1 = self.m2.proposals.create(
            body="At least one banana for each monkey"
        )
    ...
    def test_get_selected_motions_qs(self):
        ...
        self.assertEqual(
            [self.m1, self.m2], list(self.mp.get_selected_motions_qs().all())
        )
        self.m1.publish()
        self.m1.save()
    ...
    def test_populate_meeting(self):
        ...
        self.assertEqual([self.jane, self.tarzan], [x.author for x in qs.all()])
        self.assertEqual(
